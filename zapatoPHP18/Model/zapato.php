...
class zapato{
    ...
    public function listar(){
        try{
          $query = " SELECT z.id_zapato,z.precio,z.color,e.estilo,t.talla,g.genero,z.cantidad FROM dbozapato z INNER JOIN dboestilo e on z.id_estilo = e.id_estilo 
...
public function registrar(zapato $data){
  try {
      $query = "Insert into dbozapato (precio,color,id_estilo,id_talla,id_genero,cantidad)
          values (?,?,?,?,?,?)";
      $this->CNX->prepare($query)->execute(array($data->precio,$data->color,$data->id_estilo,
          $data->id_talla,$data->id_genero,$data->cantidad));
...
public function actualizarDados($data){
  try {
      $query = "Update dbozapato set precio=?, color=?, id_estilo=?, id_talla=?, id_genero=?, cantidad=?
          WHERE id_zapato = ?";
      $this->CNX->prepare($query)->execute(array($data->precio,$data->color,$data->id_estilo,
          $data->id_talla,$data->id_genero,$data->cantidad,$data->id_zapato));
